...
RUN apt-get update && \
  apt-get install -y --no-install-recommends net-tools iproute2 numactl \
  tmux bridge-utils file wget genisoimage && \
  apt-get install -y --no-install-recommends build-essential git ca-certificates \
  libqtcore4 libusbredirhost1 qtcore4-l10n spice-client-glib-usb-acl-helper \
  sshpass openssh-client rsync psmisc \
  ...
  && git clone -b v2015.11 https://github.com/SnabbCo/snabbswitch.git \
